...
import de.hterhors.obie.ml.templates.KnowledgeBaseTemplate.Scope;
 ...
 */
public class KnowledgeBaseTemplate extends AbstractOBIETemplate<Scope> {
	...
	final private static String query2 = "select distinct ?" + propertyName1 + " ?" + propertyName2 + " where {<%s> ?"
			+ propertyName1 + " ?o. ?o ?" + propertyName2 + " <%s> } ";

	final private static String query3 = "select distinct ?" + propertyName1 + " ?" + propertyName2 + " where {<%s> ?"
			+ propertyName1 + " ?o. <%s> ?" + propertyName2 + " ?o } ";

	private final TripleStoreDatabase db;

	public KnowledgeBaseTemplate(RunParameter parameter) {
		super(parameter);
	...
	class Scope extends FactorScope {
		...
		public Scope(Class<? extends IOBIEThing> entityRootClassType, AbstractIndividual value1, String slotName,
			...
			super(KnowledgeBaseTemplate.this, entityRootClassType, value1, value2, slotName, interSlotComparison);
			this.value1 = value1;
			this.value2 = value2;
	...
	public void computeFactor(Factor<Scope> factor) {
		...
		doubleProp(factor, query2, "Q2", value1Filler, value2Filler, factor.getFactorScope().interSlotComparison);
		doubleProp(factor, query3, "Q3", value1Filler, value2Filler, factor.getFactorScope().interSlotComparison);
