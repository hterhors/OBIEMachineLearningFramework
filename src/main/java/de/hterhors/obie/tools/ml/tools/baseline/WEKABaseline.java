package de.hterhors.obie.tools.ml.tools.baseline;
...
import de.hterhors.obie.tools.ml.scorer.IExternalScorer;
import de.hterhors.obie.tools.ml.scorer.InstanceCollection;
import de.hterhors.obie.tools.ml.scorer.LibSVMRegressionScorer;
...
public class WEKABaseline {
	...
	public static Logger log = LogManager.getLogger(WEKABaseline.class);
	 ...
	 * 
	 */
	private EExplorationMode explorationMode = EExplorationMode.LINEAR;
	 ...
	 *
	 */
	private final int numOfExploration = 20;
	 ...
	 */
	private static final String arffBaseDirectory = "/baseline/weka/arff/";

	private static final String modelBaseDirectory = "/baseline/weka/model/";
	...
	public WEKABaseline(OBIERunParameter parameter) throws Exception {
		...
		if (!new File(parameter.rootDirectory + arffBaseDirectory).exists()) {
			new File(parameter.rootDirectory + arffBaseDirectory).mkdirs();
		}

		final String modelName = buildModelName();

		collectTrainingData();
		System.out.println("Number of training data = " + trainingData.getDataPoints().size());

		System.out.println("Convert to ARFF...");
		Instances wekaInstance = convertToARFF("Test", trainingData);
		System.out.println("WEKA Instance: " + wekaInstance.toSummaryString());
		System.out.println("Save ARFF...");
		saveArff(new File(parameter.rootDirectory + arffBaseDirectory + modelName + ".arff"), wekaInstance);
		...
		// weka.core.SerializationHelper.read("/some/where/j48.model");

		System.out.println("Train RandomForest...");
		rf.buildClassifier(wekaInstance);
		...
		// }
		//
		// evaluate(corpusProvider.getTestCorpus().getInstances(), model);
	...
	private void collectTrainingData() throws IOException {
...
//		}
	...
	private void explore(final int numOfExploration, final int depth) {
		...
		corpusProvider.getTrainingCorpus().getInternalInstances().parallelStream().forEach(trainInstance -> {
			...
			case EXPONENTIAL:
				...
				break;
			case LINEAR:
				exploreTrainingLinear(previousStates, gold, numOfExploration, depth);
				break;
	...
	private void exploreTrainingExplonential(OBIEState previousState, List<IOBIEThing> gold, final int numOfExploration,
		...
		selectedStates.forEach(newState -> {
			...
			if (trainingData.getDataPoints().size() % 1000 == 0)
				log.debug("Collected #of datapoints: " + trainingData.getDataPoints().size());
			/*
			 * Call recursive
			 */
	...
	private void exploreTrainingLinear(final List<OBIEState> previousStates, List<IOBIEThing> gold,
		...
		previousStates.forEach(newState -> {
			...
			if (trainingData.getDataPoints().size() % 10000 == 0)
				log.info("Collected #of datapoints: " + trainingData.getDataPoints().size());
		 ...
		 * Call recursive
		 */
		exploreTrainingLinear(selectedStates, gold, numOfExploration, remainingCalls - 1);
	...
	/**
	 * Converts an InternalInstanceCollection into WEKA ARFF.
	 * 
	 * @param instanceCollection
	 */
	private Instances convertToARFF(final String dataSetName, final InstanceCollection instanceCollection) {
		List<String> binaryValues = new ArrayList<>();
		binaryValues.add("1");
		binaryValues.add("0");

		Attribute[] attributes = new Attribute[instanceCollection.sparseIndexMapping.size()];
		for (Entry<String, Integer> attribute : instanceCollection.sparseIndexMapping.entrySet()) {
			attributes[attribute.getValue()] = new Attribute(attribute.getKey(), binaryValues);
		}

		ArrayList<Attribute> attributeList = new ArrayList<>(Arrays.asList(attributes));

		Attribute classAttribute = new Attribute("class");

		attributeList.add(classAttribute);

		Instances instances = new Instances(dataSetName, attributeList, instanceCollection.getDataPoints().size());
		instances.setClassIndex(attributeList.size() - 1);

		for (FeatureDataPoint fdp : instanceCollection.getDataPoints()) {
