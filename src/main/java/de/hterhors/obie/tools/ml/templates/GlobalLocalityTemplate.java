...
import de.hterhors.obie.tools.ml.templates.GlobalLocalityTemplate.Scope;
...
public class GlobalLocalityTemplate extends AbstractOBIETemplate<Scope> {
	...
	public GlobalLocalityTemplate(OBIERunParameter parameter) {
		super(parameter);
	}
	...
	private static Logger log = LogManager.getFormatterLogger(GlobalLocalityTemplate.class.getName());
	...
	private void addFeatures(Vector featureVector, Class<? extends IOBIEThing> class1Type,
		...
		for (final int localityDist : localityDistances) {
			...
			featureVector.set("[" + class1Name + "_" + class2Name + "] min token dist < " + localityDist,
